...
- [Publishing as a static site](#publishing-as-a-static-site)
...
BrainKu works as a flat git repo, but if you want to **publish it as a browsable
website** (for sharing, mobile reading, or full-text search), render it with
[MkDocs Material](https://squidfunk.github.io/mkdocs-material/) and serve via
[GitHub Pages](https://pages.github.com/).

### Why MkDocs Material

- Native markdown rendering (no conversion step).
- `[[wikilinks]]` rendered as clickable cross-references.
- Full-text search across all pages.
- Light/dark mode, instant navigation, mobile-responsive.
- GitHub Pages is free, public, HTTPS-enforced.

### Setup

**Layout change required:** MkDocs requires `docs_dir` to be a child of the config file,
not a parent. Move all wiki content into a `docs/` subdirectory.
...
├── README.md                   # repo landing page (slim)
└── docs/                       # ALL wiki content lives here
    ├── index.md                # home/welcome page (auto-routed to /)
    ├── schema.md
...
**Critical:** `docs/index.md` is auto-routed to site root `/`. The content catalog
must be renamed to `docs/catalog.md` to avoid the clash.

### One-time setup

```bash
# 1. Install locally (in a venv) for testing
cd /root/dev/BrainKu
python3 -m venv .venv && source .venv/bin/activate
pip install mkdocs mkdocs-material pymdown-extensions

# 2. Create mkdocs.yml at the repo root (see templates or examples online)
# - docs_dir: docs
# - theme: material
# - nav: explicit block mirroring wiki section structure
...
Welcome. This is the published view of the BrainKu knowledge base.
...
# 4. Preview locally
mkdocs serve
# Opens at http://127.0.0.1:8000/BrainKu/

# 5. Verify the build is clean
mkdocs build --strict
# --strict fails on any warning — catches broken wikilinks, missing pages.

# 6. Push to GitHub
git add -A
git commit -m "Restructure for MkDocs publishing"
git push origin main

# 7. Set up GitHub Pages
# - Create the site via API (admin scope required):
gh api --method POST -H "X-GitHub-Api-Version: 2022-11-28" \
  repos/<owner>/BrainKu/pages \
  -f source[branch]=main -f source[path]=/
# - Switch to workflow build type:
...
After this, the wiki is live at `https://<owner>.github.io/BrainKu/`.

### Optional: Obsidian-like UI

The published site is functional but plain. For an Obsidian-like experience
(file tree sidebar, hover previews, per-page local graph, etc.), see the
`mkdocs-obsidian-ui.md` reference in the Hermes `llm-wiki` skill.
